...
jar.name=HW3-1.0-SNAPSHOT.jar
jar.path=target/${jar.name}


#######################################################################TA CONFIGURE HERE############
job.name=driver.DriverProgram
local.input=  ### MENTION INPUT PATH
local.output=### MENTION OUTPUT PATH
#######################################################################TA CONFIGURE  HERE############

# Pseudo-Cluster Execution
hdfs.user.name=manthan
hdfs.input=input1
hdfs.output=topKResults


# AWS EMR Execution
aws.emr.release=emr-5.2.1
aws.region=us-east-1
aws.bucket.name=inputdatasetmapreduce
aws.subnet.id=subnet-612a0f05
aws.input=input1
aws.output=output
aws.log.dir=log
aws.num.nodes=11
...
clean-local-output:
	cp -vR ${local.output} ${local.input} ; rm -rf ${local.input}.* ; rm -rf ${local.output}
...
cloud: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "6 Large machines small data set" \
